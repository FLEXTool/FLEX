...
@property (nonatomic, readonly) UISwitch *excludeImagesSwitch;
@property (nonatomic, readonly) UISwitch *excludeAnalyticsSwitch;
@property (nonatomic, readonly) UISwitch *excludeFirebaseAnalyticsSwitch;
@end
...
- (void)viewDidLoad
{
    ...
    // Export filter switches
    _excludeImagesSwitch = [UISwitch new];
    _excludeAnalyticsSwitch = [UISwitch new];
    _excludeFirebaseAnalyticsSwitch = [UISwitch new];

    self.excludeImagesSwitch.on = defaults.flex_exportExcludeImages;
    [self.excludeImagesSwitch addTarget:self
                                 action:@selector(excludeImagesToggled:)
                       forControlEvents:UIControlEventValueChanged];

    self.excludeAnalyticsSwitch.on = defaults.flex_exportExcludeAnalytics;
    [self.excludeAnalyticsSwitch addTarget:self
                                    action:@selector(excludeAnalyticsToggled:)
                          forControlEvents:UIControlEventValueChanged];

    self.excludeFirebaseAnalyticsSwitch.on = defaults.flex_exportExcludeFirebaseAnalytics;
    [self.excludeFirebaseAnalyticsSwitch addTarget:self
                                            action:@selector(excludeFirebaseAnalyticsToggled:)
                                  forControlEvents:UIControlEventValueChanged];
}
...
- (void)excludeImagesToggled:(UISwitch *)sender
{
    [NSUserDefaults standardUserDefaults].flex_exportExcludeImages = sender.isOn;
}

- (void)excludeAnalyticsToggled:(UISwitch *)sender
{
    [NSUserDefaults standardUserDefaults].flex_exportExcludeAnalytics = sender.isOn;
}

- (void)excludeFirebaseAnalyticsToggled:(UISwitch *)sender
{
    [NSUserDefaults standardUserDefaults].flex_exportExcludeFirebaseAnalytics = sender.isOn;
}
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    switch (section) {
        ...
        case 1:
            return 3;
        case 2:
            return self.hostDenylist.count;
        default:
            return 0;
    }
}
...
- (NSString *)tableView:(UITableView *)tableView titleForFooterInSection:(NSInteger)section
{
    ...
    if (section == 1) {
        return @"These filters apply when exporting requests. "
                "Analytics includes 80+ tracking SDKs (Adjust, Facebook, AppsFlyer, etc.). "
                "Firebase Analytics excludes FA but keeps Remote Config.";
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    switch (indexPath.section) {
        ...
        case 1: {
            switch (indexPath.row) {
                case 0:
                    cell.textLabel.text = @"Exclude Images";
                    cell.accessoryView = self.excludeImagesSwitch;
                    break;
                case 1:
                    cell.textLabel.text = @"Exclude Analytics SDKs";
                    cell.accessoryView = self.excludeAnalyticsSwitch;
                    break;
                case 2:
                    cell.textLabel.text = @"Exclude Firebase Analytics";
                    cell.accessoryView = self.excludeFirebaseAnalyticsSwitch;
                    break;
            }
            break;
        }
...
{
    // Can only select the "Reset Host Denylist" row (was row 2, now row 3 since we added View JSON)
